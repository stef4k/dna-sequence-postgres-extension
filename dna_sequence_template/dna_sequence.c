...
spg_kmer_choose(PG_FUNCTION_ARGS)
{
    ...
    /* Check for prefix match and set nodeChar to the next character after the prefix */
    if (in->hasPrefix)
    {
        text       *prefixKmer = DatumGetTextPP(in->prefixDatum);
        prefixStr = VARDATA_ANY(prefixKmer);
        ...
        if (commonLen == prefixSize)
        {
            if (inSize - in->level > commonLen)
                ...
                nodeChar = -1;  // End of string case
        }
        else
        {
            /* Perform prefix splitting */
            out->resultType = spgSplitTuple;
            ...
            /* Create labels for split nodes */
            out->result.splitTuple.prefixNodeLabels[0] =
                Int16GetDatum(*(unsigned char *) (prefixStr + commonLen));
            out->result.splitTuple.prefixNodeLabels[1] =
                Int16GetDatum(nodeChar);

            /* Set the child node index */
            out->result.splitTuple.childNodeN = (inSize > commonLen) ? 1 : 0;

            /* Handle the postfix for the lower-level tuple */
            if (prefixSize - commonLen > 1)
            {
                out->result.splitTuple.postfixHasPrefix = true;
                out->result.splitTuple.postfixPrefixDatum =
    ...
    {
        /* Handle 'allTheSame' by creating a single child node */
        out->resultType = spgSplitTuple;
        out->result.splitTuple.prefixHasPrefix = in->hasPrefix;
        out->result.splitTuple.prefixPrefixDatum = in->prefixDatum;
        out->result.splitTuple.prefixNNodes = 1;
    ...
    {
        /* Add a new child node for nodeChar */
